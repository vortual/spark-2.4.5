...
private[spark] class ApplicationMaster(args: ApplicationMasterArguments) extends Logging {
  ...
  private def runDriver(): Unit = {
    ...
    try {
      ...
      if (sc != null) {
        ...
        /* sparkcontext 的 conf 默认如果没有在 new sparkcontext 的时候
        传入 new sparkconf() 的话，
        会从系统属性里面取。对应代码：org.apache.spark.SparkConf.loadFromSystemProperties
         系统里面的 conf 是在上面设置进去的，通过读取运行 ApplicationMaster 类时传入的 __spark_conf__.properties 文件，里面有 executor memory 等参数，代码如下：
          private val sparkConf = new SparkConf()
          if (args.propertiesFile != null) {
            Utils.getPropertiesFromFile(args.propertiesFile).foreach { case (k, v) =>
              sparkConf.set(k, v)
  ...
  private def runExecutorLauncher(): Unit = {
    ...
    // zrm: yarn-client 分配 executor container 的地方
